//
//  WeatherCodeMapper.swift
//  RainStorm
//
//  Maps Open-Meteo weathercode to icon names and descriptions
//  Copyright © 2019 Guima Games. All rights reserved.
//

import Foundation

enum WeatherCodeMapper {
    
    /// Maps Open-Meteo weathercode to existing icon asset names
    static func iconName(for code: Int) -> String {
        switch code {
        case 0:
            return "clear-day"
        case 1, 2, 3:
            return "cloudy"
        case 45, 48:
            return "fog"
        case 51, 53, 55, 56, 57:
            return "rain" // Drizzle
        case 61, 63, 65, 66, 67:
            return "rain" // Rain
        case 71, 73, 75, 77:
            return "snow"
        case 80, 81, 82:
            return "rain" // Rain showers
        case 85, 86:
            return "snow" // Snow showers
        case 95, 96, 99:
            return "rain" // Thunderstorm
        default:
            return "clear-day"
        }
    }
    
    /// Returns a human-readable description for the weathercode
    static func description(for code: Int) -> String {
        switch code {
        case 0:
            return "Clear sky"
        case 1:
            return "Mainly clear"
        case 2:
            return "Partly cloudy"
        case 3:
            return "Overcast"
        case 45:
            return "Fog"
        case 48:
            return "Depositing rime fog"
        case 51:
            return "Light drizzle"
        case 53:
            return "Moderate drizzle"
        case 55:
            return "Dense drizzle"
        case 56:
            return "Light freezing drizzle"
        case 57:
            return "Dense freezing drizzle"
        case 61:
            return "Slight rain"
        case 63:
            return "Moderate rain"
        case 65:
            return "Heavy rain"
        case 66:
            return "Light freezing rain"
        case 67:
            return "Heavy freezing rain"
        case 71:
            return "Slight snow fall"
        case 73:
            return "Moderate snow fall"
        case 75:
            return "Heavy snow fall"
        case 77:
            return "Snow grains"
        case 80:
            return "Slight rain showers"
        case 81:
            return "Moderate rain showers"
        case 82:
            return "Violent rain showers"
        case 85:
            return "Slight snow showers"
        case 86:
            return "Heavy snow showers"
        case 95:
            return "Thunderstorm"
        case 96:
            return "Thunderstorm with slight hail"
        case 99:
            return "Thunderstorm with heavy hail"
        default:
            return "Unknown"
        }
    }
}
